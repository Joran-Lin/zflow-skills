...
What's in foreground, midground, and background. Every beat should have at least 2 layers, and the background must have texture — not just a color + glow:

- "BG: dark navy fill + flowing particle field (CSS dots drifting rightward) + gradient mesh (two overlapping radial glows, purple and cyan). MG: stat cards with drop shadow + hero number at 140px. FG: accent rule + corner registration marks + mono timestamp."

- "BG: duotone video at 50% (purple/cyan grade) + noise grain overlay. MG: heading panel (left, dark surface) + full-frame image with color treatment (right). FG: data stream labels along left edge + thin cyan divider."

- "BG: hex grid pattern at 12% + oversized ghost type at 15%. MG: CSS donut chart (center, animated) + three data cards (below). FG: amber status dot + mono category labels."

Thin backgrounds to avoid: "dark fill + subtle glow" — a glow is a colored spot, not a texture. The viewer should see something interesting in the background even before the content appears.
...
When a single beat (scene) spans multiple narration blocks, it needs internal phasing — the visual must evolve as the narration progresses, not freeze after the initial entrance.

**Sub-beat = phase boundary.** Each block within a multi-block scene is a sub-beat. The phase transition at each sub-beat is a rhythm event — not just "more stuff appears," but a deliberate visual change that the viewer perceives as a beat within the beat.

**Rhythm of phase transitions:**
- A phase transition is not a scene cut — it's an evolution. At least one existing element MUST visually respond (transform, shift, pulse, change color) when the new phase arrives. This creates a sense that the frame itself is alive and reacting to the narration.
- Phase timing derives from `block-timing.json` `startTime` values — block-level precision, not word-level.

**Cross-validation with Visual Density Gate:**
- Every sub-beat's visible frame must satisfy the density gate's core spirit (three layers, focal weight, frame anchoring). If hiding Phase 2+ elements makes Phase 1 look hollow, Phase 1 needs more elements — either unique to it or temporarily present before Phase 2 transforms them.
- Multi-block scenes should have more total elements than single-block scenes of equal duration — each phase contributes its own visual content, not just a redistribution of a fixed pool.
