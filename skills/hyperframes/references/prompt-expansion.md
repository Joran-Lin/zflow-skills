...
4. **Per-scene beats** — for each scene, use the beat-direction format:
   ...
   - **Depth layers** — BG (2-5 decoratives with ambient motion), MG (content), FG (accents, structural elements, micro-details). 8-10 total elements per scene per video-composition.md. **Non-text elements must be the visual majority.** body-text appears ONLY when it adds information not in the narration; if it mirrors the caption, use a visual element instead (stat pill, progress bar, comparison chart, icon-label row, tag badge, breakdown bar).
   - **Information elements** — list the 3-5 key information points this scene conveys, and map each to a specific visual carrier (stat card → "3亿", comparison bar → "60% vs 40%", tier row → "助餐✓ 助医✗ 康复✗"). If you can't map an information point to a visual, either add one or question whether the point belongs in this scene. Decoration provides atmosphere; information provides substance. Both are required.
