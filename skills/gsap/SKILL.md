...
name: gsap
description: >
  GSAP animation reference for HyperFrames. Covers gsap.to()/from()/fromTo(),
  easing, stagger, defaults, timelines (gsap.timeline(), position parameter,
  labels, nesting, playback control), and performance optimization (transform,
  will-change, quickTo). Consult as needed when writing GSAP animations in a
